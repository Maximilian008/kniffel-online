      ...
      if (next > prev) {
        for (let i = prev; i < next; i += 1) {
          ...
          if (!room.players[role]) {
            room.players[role] = createPlayerSlot(role, i, room.state.playerNames[i]);
          }
        }
      } else if (next < prev) {
...
function loadOrCreateRoom(roomId: string): GameRoom {
  ...
  const fallbackCapacity = initialState.playerNames.length || 2;
  const capacity = Math.max(2, fallbackCapacity);
  const room = buildGameRoom({
    id: roomId,
    capacity,
    state: initialState,
...
function ensureRoomSlots(room: GameRoom) {
  for (let i = 0; i < room.capacity; i += 1) {
    const role = roleByIndex(i);
    if (!room.players[role]) {
      room.players[role] = createPlayerSlot(role, i, room.state.playerNames[i]);
